var config = {
  ...
  default_uri: "",
  url_slug: "/pages/multicore-configurator",
  item_property_slug: "Custom Multicore Cable",
};
var orderLines = [];
var orderLinesData = [];
...
function showChannels(numOfChannels)
{
  console.log("show " + numOfChannels + " channels");

  $('body').removeClass("channels_2");
  $('body').removeClass("channels_4");
  $('body').removeClass("channels_8");
  $('body').removeClass("channels_12");
  $('body').removeClass("channels_24");
  $('body').removeClass("channels_48");

  $('body').addClass("channels_" + numOfChannels);

  var $channels = $("#select_sideA_channels");
  var $channelsB = $("#select_sideB_channels");

  var $channelConnectors = $('#multicore_sideA_channel_connectors');
  var $channelConnectorsB = $('#multicore_sideB_channel_connectors');

  $channels.html("");
  $channelsB.html("");
  $channelConnectors.html("");
  $channelConnectorsB.html("");

  for(var i=1; i<=numOfChannels; i++)
  {
    var channelChooserHTML = '<li class="" data-show="#multicore_sideA_channel'+i+'_connector"><a href="#"><span>'+i+'</span></a></li>';
    $channels.append(channelChooserHTML);

    var channelChooserHTMLB = '<li class="" data-show="#multicore_sideB_channel'+i+'_connector"><a href="#"><span>'+i+'</span></a></li>';
    $channelsB.append(channelChooserHTMLB);

    var connectorHTML = $('#multicore_channel_connector_template').html();
    connectorHTML = connectorHTML.replaceAll("sideX_channelX", "sideA" + "_channel" + i);
    $channelConnectors.append(connectorHTML);

    var connectorHTML2 = $('#multicore_channel_connector_template').html();
    connectorHTML2 = connectorHTML2.replaceAll("sideX_channelX", "sideB" + "_channel" + i);
    $channelConnectorsB.append(connectorHTML2);
