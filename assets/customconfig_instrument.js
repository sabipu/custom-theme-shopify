var config = {
  ...
  default_uri: "c=1&cable-brand=beldon&cable-type=beldon_9778&cable-length=10&connectorA-brand=neutrik&connectorA-connector=connector_neutrik_np2x_b&connectorA-boot=neutrik_bootColor_black&connectorB-brand=neutrik&connectorB-connector=connector_neutrik_np2x_b&connectorB-boot=neutrik_bootColor_black",
  url_slug: "/pages/instrument-configurator",
  item_property_slug: "Custom Instrument Cable",
...
function loadPrefillProduct(obj)
{
  ...
  // forceWindowToScrollTop(function ()
  // {
  //   $image.css("opacity", 1);
...
function changeImage (imageOptions)
{
  ...
  if(imageOptions.group == "cable" && imageOptions.slug != image[imageOptions.group] && typeof image["sleeve"] == "undefined")
  {
    ...
    // $('#cable_side_b_cable').removeClass($('#cable_side_b_cable').attr("data-current-class"));
    // $('#cable_side_a_cable').removeClass($('#cable_side_a_cable').attr("data-current-class"));
   
    // $('#cable_side_b_cable').attr("data-current-class", imageOptions.slug);
    // $('#cable_side_a_cable').attr("data-current-class", imageOptions.slug);
    
    // $('#cable_side_b_cable').addClass(imageOptions.slug);
    // $('#cable_side_a_cable').addClass(imageOptions.slug);
    
    toggleAndStoreClassForUpdateImage($('#cable_side_b_cable'), imageOptions.slug, "productImage_cable_instrument", imageOptions);
    toggleAndStoreClassForUpdateImage($('#cable_side_a_cable'), imageOptions.slug, "productImage_cable_instrument", imageOptions);

    
    // console.log("cable image slug: " + imageOptions.slug);
    $('#cable_side_b_cable').css({"background-image": "url("+config.image_url+"images/cfg/"+ imageOptions.slug + ".png)"});
    $('#cable_side_a_cable').css({"background-image": "url("+config.image_url+"images/cfg/"+ imageOptions.slug + ".png)"});
    cableSet = true;
  }
  ...
  if(imageOptions.group == "connectorA" && imageOptions.slug != image[imageOptions.group])
  {
    console.log("connector A image slug: " + imageOptions.slug);
    
    // $('#cable_side_a_connector').removeClass($('#cable_side_a_connector').attr("data-current-class"));
    // $('#cable_side_a_connector').attr("data-current-class", imageOptions.slug);
    // $('#cable_side_a_connector').addClass(imageOptions.slug);

    toggleAndStoreClassForUpdateImage($('#cable_side_a_connector'), imageOptions.slug, "productImage_cable_instrument", imageOptions);
  ...
  if(imageOptions.group == "connectorB" && imageOptions.slug != image[imageOptions.group])
  {
    console.log("connector B image slug: " + imageOptions.slug);
    
    // $('#cable_side_b_connector').removeClass($('#cable_side_b_connector').attr("data-current-class"));
    // $('#cable_side_b_connector').attr("data-current-class", imageOptions.slug);
    // $('#cable_side_b_connector').addClass(imageOptions.slug);

    toggleAndStoreClassForUpdateImage($('#cable_side_b_connector'), imageOptions.slug, "productImage_cable_instrument", imageOptions);
...
function chooseableClicked(el)
  {
    ...
    if($closest.length > 0) {
      if($el.data('url-value') === 'sommer_sc_llx') {
    ...
    if($el.hasClass("one_side_only"))
    {
      console.log("HAS ONE SIDE ONLY");
      if($el.hasClass("one_side_only_a"))
      {
        $(".one_side_only_b").hide();
      }
      else if($el.hasClass("one_side_only_b"))
      {
        $(".one_side_only_a").hide();
      }
    }
    else
    {
